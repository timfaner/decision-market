...
//     signal base;
//     base <-- (1 << n);
//     component baseBits = Num2Bits(n+1);
//     baseBits.in <== base;
//     for(var i=0;i<n;i++){
//         baseBits.out[i] * baseBits.out[i] === 0;
//     }
//     baseBits.out[n] * baseBits.out[n] === 1;


//     out[0] <-- in[0] % base;
//     carry[0] <-- in[0] \ base;
//     carry[0] * base + out[0] === in[0];

//     for(var i=1; i<k;i++){
//         var tmp = (in[i] + carry[i-1]);
//         out[i] <-- tmp % base;
//         carry[i] <-- tmp \ base;
//         tmp === carry[i] * base + out[i];
//     }
//     out[k] <== carry[k-1];

...
contract ZkEvoteHTLP is registerCommitments, Verifier {
    ...
    function initialize(
        ...
        // NghHash = PoseidonT4.poseidon([wordsHasher(_puzzle.N), wordsHasher(_puzzle.g), wordsHasher(h_pow_N_words)]);
    }
    ...
    function castVote(
        ...
        uint256[] memory _publicSignals = new uint256[](67);
        for (uint8 i = 0; i < 9; i++) {
            _publicSignals[i] = puzzle.N[i];
        }
        for (uint8 i = 9; i < 18; i++) {
            _publicSignals[i] = puzzle.g[i - 9];
        }
        for (uint8 i = 18; i < 36; i++) {
            _publicSignals[i] = puzzle.h_pow_N[i - 18];
        }
        _publicSignals[36] = registerCommitments.root;
        _publicSignals[37] = uint256(uint160(msg.sender));
        _publicSignals[38] = _fee;

        for (uint8 i = 39; i < 48; i++) {
            _publicSignals[i] = vData.u[i - 39];
        }
        for (uint8 i = 48; i < 66; i++) {
            _publicSignals[i] = vData.v[i - 48];
        }
        _publicSignals[66] = vData.nullifierHash;
        require(
            Verifier.zkVerify(
                vProof.a,
                vProof.b,

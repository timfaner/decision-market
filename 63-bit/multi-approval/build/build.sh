...
# Main()
useMessage="Usage: ./build.sh -n ${GREEN}[NUMBER_of_VOTERS]${NC} -m ${GREEN}[NUMBER_of_CANDIDATES]${NC} -a ${GREEN}[NUMBER_of_APPROVAL]${NC}"
# Parsing arguments
while getopts "hn:m:a:" opt
do
   case "$opt" in
      n)
         nVotersOpt="$OPTARG";
...
k=$(python3 -c "import math; print(math.floor(math.log2($nVoters))+1)")
...
if [[ $((k*nCandiadates)) -ge "63" ]]; then
   echo "${GREEN}[NUMBER_of_CANDIDATES]${NC} * (1 + log2(${GREEN}[NUMBER_of_VOTERS]${NC})) must be less than 63";
   exit 1
fi
...
sed -i.bak "s/__nApproves__/$nApproves/g" ../circuits/castVote_main.circom
sed -i.bak "s/__nApproves__/$nApproves/g" ../test/2_eVote.test.js
